...
if [ "$(nasm -v)" = "" ] || [ "$(nasm -v | grep Apple)" != "" ]; then
  ...
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/nasm-mac64.zip" || exit 1
  nasmzip=$(cat nasm-mac64.zip)
  rm -rf nasm-*
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/${nasmzip}" || exit 1
...
if [ "$(iasl -v)" = "" ]; then
  ...
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/iasl-macosx.zip" || exit 1
  iaslzip=$(cat iasl-macosx.zip)
  rm -rf iasl
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/${iaslzip}" || exit 1
...
if [ "${MTOC_HASH}" = "" ]; then
  MTOC_HASH=$(curl -L "https://github.com/mikebeaton/ocbuild/raw/master/external/mtoc-mac64.sha256") || exit 1
fi
...
if ! $valid_mtoc; then
  ...
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/mtoc-mac64.zip" || exit 1
  mtoczip=$(cat mtoc-mac64.zip)
  rm -rf mtoc-*
  curl -OL "https://github.com/mikebeaton/ocbuild/raw/master/external/${mtoczip}" || exit 1
...
if [ "$NEW_BUILDSYSTEM" != "1" ]; then
  if [ "$OFFLINE_MODE" != "1" ]; then
    updaterepo "https://github.com/mikebeaton/audk" UDK master || exit 1
  else
